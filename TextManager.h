...
public:
	FontManager(){}
	void add_font(const char* fontPath, bool isSDF, std::string fontName, std::unique_ptr<Engine::Graphics::RendererInterface>& renderer) {
		...
		font.texture = renderer->createR8TextureFromData(img_data, imgWidth, imgHeight, false);
		delete[] img_data;
		stbi_image_free(image_bytes);
	}
	Font& get_font(std::string fontName) {
...
class TextManager {
public:
	TextManager(std::unique_ptr<Engine::Graphics::RendererInterface>& renderer);
	~TextManager();

	void setup_fonts(std::unique_ptr<Engine::Graphics::RendererInterface>& renderer);
	void add_font(const char* fontPath, bool isSDF, std::string fontName, std::unique_ptr<Engine::Graphics::RendererInterface>& renderer);
	

	//update buffer for just one single text
	void update_text(const char* text, float letter_height, glm::vec3 bottom_left_position, glm::vec4 text_color, std::string fontName);
	void update_centered_text(const char* text, float letter_height, glm::vec3 center_position, glm::vec4 text_color, std::string fontName);

	//allows to add any kind of text to buffer, then render all added text with 1 draw call
	void add_text_to_buffer(const char* text, float letter_height, glm::vec3 bottom_left_position, glm::vec4 text_color, std::string fontName);
	void add_centered_text_to_buffer(const char* text, float letter_height, glm::vec3 center_position, glm::vec4 text_color, std::string fontName);
