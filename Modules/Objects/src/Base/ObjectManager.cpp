...
Engine::Object::object_ptr Engine::Object::ObjectManager::InstantiateObject(uint32_t obj_id) {
	objects.add(Object(obj_id), obj_id);
	return object_ptr(obj_id);
}

void Engine::Object::ObjectManager::DestroyObject(uint32_t obj_id) {
	auto obj_ptr = objects.get(obj_id);
	if(obj_ptr)
		obj_ptr->Destroy();
}
