...
struct DrawElementsIndirectCommand {
	...
	DrawElementsIndirectCommand(uint32_t indexCount, uint32_t instanceCount, uint32_t baseIndex, uint32_t baseVertex, uint32_t baseInstance) :
		indexCount{ indexCount }, instanceCount{ instanceCount }, baseIndex{ baseIndex }, baseVertex{ baseVertex }, baseInstance{ baseInstance } {}
	uint32_t indexCount{};
	uint32_t instanceCount{};
	uint32_t baseIndex{};    //first index in index buffer
...
namespace Engine::Graphics {
	...
	public:
		...
		virtual std::unique_ptr<Texture> createTextureFromData(const unsigned char* image_bytes, int imgWidth, int imgHeight, int numOfColorChannels, bool isPixelised) = 0;
		virtual std::unique_ptr<Texture> createR8TextureFromData(const unsigned char* image_bytes, int imgWidth, int imgHeight, bool isPixelised) = 0;
	public:
